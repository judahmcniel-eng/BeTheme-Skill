...
1. Start with native BeBuilder elements.
2. Recommend a Global Section workflow.
3. Use performance-first language: WebP, CDN, compression, caching, and reduced mobile motion.
4. Introduce Code elements only when native BeBuilder cannot accomplish the desired motion.
5. Always mention mobile fallback for heavy visual effects.
6. Mention `max_input_vars` when the page is complex or BeBuilder saving is unreliable.
7. Prefer reusable sections, presets, and global styles over one-off custom work.
8. Use Layer Navigator for diagnosing layout problems.
